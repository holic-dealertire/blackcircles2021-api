...
def lambda_handler(event, context):
    ...
    # randdate = now.strftime('%Y%m%d%H%M%S')
    # x = random.randint(10, 99)

    if 'od_id' not in event or 'od_name' not in event or 'od_tel' not in event or 'od_zip' not in event or 'od_addr1' not in event or 'od_addr2' not in event or 'od_addr3' not in event or 'od_memo' not in event or 'od_receipt_price' not in event or 'cart_item' not in event:
        return {
            'statusCode': 402,
    ...
    od_memo = event['od_memo']
    od_receipt_price = event['od_receipt_price']
    ...
    if type(od_receipt_price) is int:
        od_receipt_price = str(od_receipt_price)
    else:
        return {
            'statusCode': 402,
            'message': "parameter error"
    ...
    if type(od_name) is int or type(od_tel) is int or type(od_zip) is int or type(od_addr1) is int or type(od_addr2) is int:
        return {
            'statusCode': 402,
            'message': "parameter error"
        }
        ...
        if row_count != 0:  # ㅇㅇ
            ...
            for i in range(length):
                ...
                if io_part_no and ct_qty:
                    cursor.execute("select io_no, it_id, delivery_idx, delivery_seller_no, sale_delivery, it_name, delivery_collect, io_size, io_id, delivery_price1, io_factory_price, io_size_origin, io_pr, io_max_weight, io_speed, io_car_type, io_maker, io_oe, io_tire_type "
                                   ...
                                   "                select * from ("
                                   "                                    select * from ("
                                   "                                                        select io_no as stock_io_no, mb_no as delivery_seller_no, sale_delivery, stock, idx as delivery_idx, delivery_price as delivery_price1, delivery_collect from tbl_item_option_price_stock where stock > %s"
                                   "                                                  ) stock"
                                   "                                    left join (select io_no as check_io_no, it_id as check_io_it_id from g5_shop_item_option where io_part_no=%s) check_option on check_option.check_io_no=stock.stock_io_no"
                                   ...
                                   "                                    where contract_idx is not null"
            ...
            cursor.execute("insert into g5_shop_order set od_id=%s, mb_id=%s, od_pwd='', od_name=%s, od_email='', od_tel=%s, od_hp=%s, od_zip1=%s, od_zip2=%s, od_addr1=%s, od_addr2=%s, od_addr3=%s, od_addr_jibeon='', od_b_name=%s, od_b_tel=%s, od_b_hp=%s, od_b_zip1=%s, od_b_zip2=%s, od_b_addr1=%s, od_b_addr2=%s, od_b_addr3=%s, od_b_addr_jibeon='', od_deposit_name=%s, od_memo=%s, od_cart_count=%s, od_cart_price=%s, od_cart_coupon=0, od_send_cost=0, od_send_cost2=0, od_coupon=0, "
                           ...
                           (od_id, mb_id, od_name, od_tel, od_tel, od_zip1, od_zip2, od_addr1, od_addr2, od_addr3, od_name, od_tel, od_tel, od_zip1, od_zip2, od_addr1, od_addr2, od_addr3, od_name, od_memo, length, tot_price, od_receipt_price, nowDatetime, nowDatetime, od_reserv_date))
            connection.commit()
