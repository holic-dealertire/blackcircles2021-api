...
def lambda_handler(event, context):
    ...
    cursor.execute("SELECT io_size, io_size_origin, io_part_no, io_pr, io_max_weight, io_speed, io_car, io_oe, io_car_type, io_tire_type, io_factory_price, io_maker, it_name, it_pattern, it_season, it_performance_type, tot_stock, io_btob_price as io_price FROM   "
                   "    (SELECT io_no, it_id AS io_it_id, io_size, io_part_no, io_size_origin, io_pr, io_max_weight, io_speed, io_car, io_oe, io_tire_type, io_factory_price, io_maker, io_car_type, sell_cnt, io_btob_lowest, io_btob_price FROM  g5_shop_item_option WHERE io_part_no='" + io_part_no + "' AND origin_io_no IS NULL AND io_btob_price is not null) opt"
